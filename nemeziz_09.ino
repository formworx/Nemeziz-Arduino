...
05 - included averaging algorithm for distance semsor to settle glitches
06 - added timeout on cap sensors at 500 ms
07 - 
08 - Pedro moved calibration of position sensor to setup stage
09 - Mark removed proximity smoothing by changing numReadings to 1 so that Gus can control volume properly
*/
#include <Wire.h>
...
int bendMin = 534; //was 160
int bendMax = 700; //was 130
...
//number of readings for smoothing distance sensor
const int numReadings = 1;
int readings[numReadings];;  //for distance measure
int readIndex = 0;
int total = 0;
int distance = 0;
...
void setup(void)
{
  ...
  lastTime = millis();
   // initialize all the distance readings to 0:
  for (int thisReading = 0; thisReading < numReadings; thisReading++) {
    readings[thisReading] = 0;
  }
cs_11_10.set_CS_Timeout_Millis(500);
cs_9_8.set_CS_Timeout_Millis(500);
cs_7_6.set_CS_Timeout_Millis(500);
